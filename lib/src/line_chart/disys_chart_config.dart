...
/// Type of Disys line Chart
enum DisysChartScale {
  hourly,
  weekly,
  monthly,
  yearly,
...
///`DisysChartConfig` allows the customization of the `DisysChart` widget
class DisysChartConfig {
  ...
  ///`true` if you want to enable pinch Zoom for `disysChartScale` of date types
  /// Pinch and zoom is used to switch beetwen charts of date types
  final bool pinchZoom;

  ///If the `contentWidth` is upper than the current width then the content will be scrollable (only valid for `disysChartScale` = `CUSTOM`)
  final double? contentWidth;

  ///`true` if you want to display a vertical line on each X data point, it only works when there is one `DisysLine`.
