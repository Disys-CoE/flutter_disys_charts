...
import 'disys_line.dart';
import 'disys_chart_config.dart';
...
typedef FooterDateTimeBuilder = String Function(
    DateTime value, DisysChartScale? scaleType);

class DisysChart extends StatefulWidget {
  ///Chart configuration
  final DisysChartConfig? config;

  ///Type of Chart
  final DisysChartScale disysChartScale;

  ///Aggregation of Chart
  final DisysChartAggregation disysChartAggregation;

  ///This value is required only if the `DisysChartScale` is `DisysChartScale.custom`
  ///and these values must be sorted in increasing way (These will be showed in the Axis X).
  final List<double>? xAxisCustomValues;

  ///[Optional] This callback only works if the `DisysChartScale` is `DisysChartScale.custom` otherwise it will be ignored
  ...
  ///[Optional] This callback only works if the `DisysChartScale` is Date type otherwise it will be ignored
  ///This is used to display a custom footer value based on the current 'x' value
  ...
  ///Only works when the `DisysChartScale` is not `DisysChartScale.custom`
  ...
  ///Only works when the `DisysChartScale` is date type
  final ValueChanged<DateTime?>? onDateTimeSelected;

  ///This value is required only if the `DisysChartScale` is not `DisysChartScale.custom`
  ...
  ///For `DisysChartScale.hourly` it will use year, month, day and hour
  ///For `DisysChartScale.weekly` it will use year, month and day
  ///For `DisysChartScale.monthly` it will use year, month
  ///For `DisysChartScale.yearly` it will use year
  final DateTime? selectedDate;

  ///This value represents the value selected to display the info in the Chart
  ///It's only for `DisysChartScale.custom`
  final double? selectedValue;

  ///Disyss used in the Axis Y
  final List<DisysLine> series;

  ///Notify if the `DisysChartScale` changed, it only works with date scales.
  final ValueChanged<DisysChartScale?>? onScaleChanged;
  ...
  DisysChart({
    ...
    this.selectedValue,
    this.disysChartAggregation = DisysChartAggregation.sum,
    required this.disysChartScale,
  ...
  @override
  DisysChartState createState() => DisysChartState();
}

@visibleForTesting
class DisysChartState extends State<DisysChart>
  ...
  ///List of `DisysLine`s used to display all lines, each line contains a list of `DataPoint`s
  List<DisysLine> computedSeries = [];
  ...
  DisysChartScale? _currentDisysChartScale;
  ...
  _checkMissingValues(DateTime newDate) {
    for (DisysLine line in widget.series) {
  ...
  ///Building the data points for the `X` axis based on the `_currentDisysChartScale`
  void _buildXDataPoints() {
    _xAxisDataPoints = [];
    _tempYValues = [];
    final scale = _currentDisysChartScale;
    if (scale == DisysChartScale.custom) {
      _xAxisDataPoints = widget.xAxisCustomValues!
          .map((val) => DataPoint<double>(value: val, xAxis: val))
          .toList();
    } else if (scale == DisysChartScale.hourly) {
    ...
    } else if (scale == DisysChartScale.weekly) {
    ...
    } else if (scale == DisysChartScale.monthly) {
    ...
    } else if (scale == DisysChartScale.yearly) {
      DateTime startDate = DateTime(
        widget.fromDate!.year,
      );
      DateTime endDate = DateTime(
  ...
  ///Calculating the size of the content based on the parent constraints and on the `_currentDisysChartScale`
  double _buildContentWidth(BoxConstraints constraints) {
    ...
    if (scale == DisysChartScale.custom) {
      ...
      if (scale == DisysChartScale.hourly) {
  ...
  _onLayoutDone(_) {
    ...
    if ((widget.selectedDate != null &&
            _currentDisysChartScale != DisysChartScale.custom) ||
        (widget.selectedValue != null &&
            _currentDisysChartScale == DisysChartScale.custom)) {
      int index = -1;
      if (_currentDisysChartScale == DisysChartScale.weekly) {
        index = _xAxisDataPoints.indexWhere((dp) =>
            areEqualDates((dp.xAxis as DateTime), widget.selectedDate!));
      } else if (_currentDisysChartScale == DisysChartScale.hourly) {
        index = _xAxisDataPoints.indexWhere((dp) =>
      ...
      } else if (_currentDisysChartScale == DisysChartScale.monthly) {
        index = _xAxisDataPoints.indexWhere((dp) =>
            (dp.xAxis as DateTime).year == widget.selectedDate!.year &&
            (dp.xAxis as DateTime).month == widget.selectedDate!.month);
      } else if (_currentDisysChartScale == DisysChartScale.yearly) {
        index = _xAxisDataPoints.indexWhere(
            (dp) => (dp.xAxis as DateTime).year == widget.selectedDate!.year);
      } else if (_currentDisysChartScale == DisysChartScale.custom) {
      ...
      if (index >= 0) {
        ...
        if (_currentDisysChartScale == DisysChartScale.custom) {
  ...
  ///Calculating the new series based on the `_currentDisysChartScale`
  _computeSeries() {
    computedSeries = [];
    _yValues = [];
    //fill data series for DateTime scale type

    if (_currentDisysChartScale == DisysChartScale.monthly ||
        _currentDisysChartScale == DisysChartScale.yearly ||
        _currentDisysChartScale == DisysChartScale.weekly ||
        _currentDisysChartScale == DisysChartScale.hourly) {
      for (DisysLine line in widget.series) {
        ...
        for (DataPoint<DateTime> dataPoint
          ...
          } else if (_currentDisysChartScale == DisysChartScale.weekly) {
            key =
        ...
        valueMap.keys.forEach(
          (key) {
            ...
            if (_currentDisysChartScale == DisysChartScale.hourly) {
        ...
        DisysLine newDisysLine = DisysLine.copy(
          disysLine: DisysLine(
        ...
        computedSeries.add(newDisysLine);
      }
    } else {
      for (DisysLine line in widget.series) {
  ...
  _onPinchZoom(double scale) {
    ...
    if (isPinchZoomActive) {
      DisysChartScale lastScale = DisysChartScale.weekly;
      if (_currentDisysChartScale == DisysChartScale.monthly) {
        lastScale = DisysChartScale.monthly;
      } else if (_currentDisysChartScale == DisysChartScale.yearly) {
        lastScale = DisysChartScale.yearly;
      }

      //when the scale is below 1 then we'll try to change the chart scale depending of the `_currentDisysChartScale`
      if (scale < 1) {
        if (_currentDisysChartScale == DisysChartScale.weekly) {
          _currentDisysChartScale = DisysChartScale.monthly;
          _previousScale = 1.5;
        } else if (_currentDisysChartScale == DisysChartScale.monthly) {
          _currentDisysChartScale = DisysChartScale.yearly;
        }
        ...
        return;
        //if the scale is greater than 1.5 then we'll try to change the chart scale depending of the `_currentDisysChartScale`
      } else if (scale > 1.5 || (isOnlyOneAxis && scale > 1.2)) {
        if (_currentDisysChartScale == DisysChartScale.yearly) {
          _currentDisysChartScale = DisysChartScale.monthly;
          _currentScale = 1.0;
  ...
  void _notifyScaleChanged(DisysChartScale lastScale) {
    if (widget.onScaleChanged != null &&
        lastScale != _currentDisysChartScale) {
      widget.onScaleChanged!(_currentDisysChartScale);
    }
  }
  ...
  @override
  void didUpdateWidget(DisysChart oldWidget) {
    /// Rebuild data points and series in case:
    /// 1. if the DisysChartScale is different from the old one
    ...
    if (oldWidget.disysChartScale != widget.disysChartScale ||
        areSeriesDifferent ||
        oldWidget.fromDate != widget.fromDate ||
        oldWidget.toDate != widget.toDate) {
      _currentDisysChartScale = widget.disysChartScale;
      _buildXDataPoints();
      _computeSeries();
    }
    ...
    return Container(
      ...
      child: Listener(
        ...
        child: GestureDetector(
          ...
          onScaleUpdate: _currentDisysChartScale != DisysChartScale.custom &&
...
//DisysChart
class _DisysChartPainter extends CustomPainter {
  final DisysChartConfig? config;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Offset? p0, p1, p2, p3;
    void _drawDisysLinePath(DisysLine line) {
      Path path = Path();
      ...
      for (int i = 0; i < xAxisDataPoints!.length; i++) {
        ...
        if (disysChartScale == DisysChartScale.custom) {
          ...
          for (DataPoint<DateTime> dp
            ...
            if (disysChartScale == DisysChartScale.hourly) {
    ...
    for (DisysLine line in reversedSeries) {
      _drawDisysLinePath(line);
    }
  ...
  String _getInfoTitleText() {
    final scale = disysChartScale;
    if (bubbleLabelValueBuilder != null && scale == DisysChartScale.custom) {
      return bubbleLabelValueBuilder!(_currentXDataPoint!.value);
    }
    if (bubbleLabelDateTimeBuilder != null &&
        scale != DisysChartScale.custom) {
  ...
  String _getFooterText(DataPoint dataPoint) {
    final scale = disysChartScale;
    if (footerValueBuilder != null && scale == DisysChartScale.custom) {
      return footerValueBuilder!(dataPoint.value);
    }
    if (footerDateTimeBuilder != null && scale != DisysChartScale.custom) {
  ...
  _getYValues(Offset p0, Offset p1, Offset p2, Offset p3, double t) {
    ...
    //source: https://stackoverflow.com/questions/8217346/cubic-disys-curves-get-y-for-given-x
  ...
  @override
  bool shouldRepaint(_DisysChartPainter oldDelegate) =>
      shouldRepaintChart! ||
...
bool _compareLengths(int currentValue, List<DisysLine> val2) {
  for (DisysLine line in val2) {
...
bool _checkCustomValues(List<DisysLine> list) {
  for (DisysLine line in list) {
    if (!_areAllPositive(
